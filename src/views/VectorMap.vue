...
export default {
  components: { GridLayout, InputRadio, SelectOption },
  setup() {
    ...
    watchEffect(() => {
      vectorLayerGroup.getLayers().forEach(layer => {
        ...
        if (layerTitle === store.state.inputRadio['vectorLayersVisibility']) layer.setVisible(true)
    ...
    function highlightCountry (e) {
      Countries.getFeatures(e.pixel).then(function (features) {   
        ...
        if (!feature) return;
        if (store.state.selectOptions['vectorSelectType']!=='Multi Select') {selectedCountry.value = {}}
    ...
    function showInfo (e) {
      ...
      if (feature !== highlight) {
        if (highlight) ecoRegionsOverlay.getSource().removeFeature(highlight)
        if (feature) ecoRegionsOverlay.getSource().addFeature(feature)
        highlight = feature
      }
    }
        ...
        switch (store.state.inputRadio['vectorLayersVisibility']) {
          case 'Countries':
            ...
            map.value.on(['click', 'pointermove'], e => {
              ...
              store.state.selectOptions['vectorSelectType'] !== 'Single Select on hover' && e.type === 'pointermove' ) {return}
              highlightCountry(e)
            })
            break;
          case 'Base Map':
